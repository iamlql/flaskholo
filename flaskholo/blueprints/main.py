...
from flaskholo.decorators import confirm_required, permission_required
from flaskholo.extensions import db, md
from flaskholo.forms.main import DescriptionForm, TagForm, CommentForm
from flaskholo.models import User, Photo, Tag, Follow, Collect, Comment, Notification, Location
from flaskholo.notifications import push_comment_notification, push_collect_notification
from flaskholo.utils import rename_image, detect_image, resize_image, redirect_back, flash_errors
...
def index():
    if current_user.is_authenticated:
        ...
        per_page = current_app.config['FLASKHOLO_PHOTO_PER_PAGE']
...
def show_notifications():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['FLASKHOLO_NOTIFICATION_PER_PAGE']
...
def get_image(filename):
    return send_from_directory(current_app.config['FLASKHOLO_UPLOAD_PATH'], filename)
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        f.save(os.path.join(current_app.config['FLASKHOLO_UPLOAD_PATH'], filename))
        img, filename_d, key_list, index_list = detect_image(filename)
        filename_s = resize_image(img, filename_d, current_app.config['FLASKHOLO_PHOTO_SIZE']['small'])
        filename_m = resize_image(img, filename_d, current_app.config['FLASKHOLO_PHOTO_SIZE']['medium'])
        photo = Photo(
            filename=filename_d,
        ...
        if len(key_list) > 0:
            for idx, key in enumerate(key_list):
                if Tag.query.filter(Tag.name == key).count() == 0:
                    tag = Tag(name=key)
                    db.session.add(tag)
                else:
                    tag = Tag.query.filter(Tag.name == key).one()
                photo.tags.append(tag)
                location = Location(xmin=index_list[idx][0], ymin=index_list[idx][1], 
                        xmax=index_list[idx][2], ymax=index_list[idx][3])
                location.pic = photo
                location.tag = tag
                category_list.append('{}: ({}  {}  {}  {});'.format(key, location.xmin, location.ymin, location.xmax, location.ymax))
...
def show_photo(photo_id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['FLASKHOLO_COMMENT_PER_PAGE']
...
def show_collectors(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['FLASKHOLO_USER_PER_PAGE']
